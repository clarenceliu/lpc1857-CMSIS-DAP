...
//   response: pointer to response data
//   return:   number of bytes in response
/*¥”÷˜ª˙¥´π˝¿¥µƒ«Î«Û∞¸∫¨MDKµƒdebuggerœ¬…Ë÷√µƒ◊Ó¥Û ±÷”£¨’‚¿Ô∂‘∆‰Ω¯––Ω‚Œˆ£¨»ª∫Û¥Ê»ÎµΩ¡À32Œªµƒclock*/
#if ((DAP_SWD != 0) || (DAP_JTAG != 0))
...
//   response: pointer to response data
//   return:   number of bytes in response
/*√¸¡Ó¿‡–Õ0x13£,£¨bit1.0…Ë÷√µÙ◊™÷‹∆⁄£¨bit2 ˝æ›Ω◊∂Œµƒ…Ë÷√*/
#if (DAP_SWD != 0)
...
//   response: pointer to response data
//   return:   number of bytes in response
/*√¸¡Ó¿‡–Õ0x04,Œ™DAP_Transfer ∫ÕDAP_TransferBlock≈‰÷√≤Œ ˝*/
...
//   response: pointer to response data
//   return:   number of bytes in response
__attribute__ ((weak)) uint32_t DAP_ProcessVendorCommand(uint8_t *request, uint8_t *response) {
